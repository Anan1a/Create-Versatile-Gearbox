...
public class AdvancedGearboxVisual extends KineticBlockEntityVisual<AdvancedGearboxBlockEntity> {
     ...
     * 更新动力源朝向
     * <p>
     * 从方块实体的 source 字段计算动力源的实际朝向
     * source 字段存储的是相对于方块位置的偏移量
     */
     ...
     * - 动态添加/删除半轴实例（根据状态变化，仅在 dirty=true 时执行）
     * - 为每个旋转实例更新速度和状态
     * <p>
     * 【性能优化】使用 dirty 标记减少不必要的状态检查
     *
     * @param partialTick 部分 tick 值（用于插值）
     */
    @Override
    ...
    protected void updateShaftInstances() {
        ...
        for (Direction direction : Iterate.directions) {
            ...
            if (state == AdvancedGearboxShaftState.OFF && existing != null) {
                keys[idx] = null;
                existing.delete();
            } else if (state != AdvancedGearboxShaftState.OFF && existing == null) {
                RotatingInstance instance = instancer.createInstance();
                instance.setup(blockEntity, direction.getAxis(), getSpeed(direction))
                        .setPosition(getVisualPosition())
                        .rotateToFace(Direction.SOUTH, direction)
     ...
     * <p>
     * Crumbling 是 Create 模组中物品被破坏时的动画效果
     * 此方法返回所有旋转实例以用于该动画
     *
